...
// Seeded from the PREVIOUS file, not empty. A run targeting one unit
// (`node scripts/build-translations.js unit3`) only calls buildUnit for that
// unit below, and only that unit's entry in `freshness.my` gets overwritten —
// every other unit's record starts as a copy of what was already true and
// stays that way unless rebuilt.
//
// Starting empty was the bug: a single-unit run wrote a freshness.json
// containing ONLY that unit, discarding every other unit's staleness record.
// The next run — even a full one — then read that truncated file as its own
// "previous" baseline, found no prior record for the missing units, and
// treated every field in them as freshly delivered. A translation that was
// genuinely stale (a real mismatch between its Burmese and the current
// English) was re-baselined as current and started rendering silently wrong —
// the exact failure this whole mechanism exists to prevent, reintroduced by
// the tool meant to maintain it.
const freshness = {
  _note: 'GENERATED by scripts/build-translations.js. Records the English each field was translated FROM, so that editing the copy marks its translation stale instead of leaving a translation of text that no longer exists. Never edit by hand.',
  my: { ...previousFreshness.my },
};

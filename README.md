...
Photographs live in `public/images/` and are referenced from the content by
filename. [`docs/IMAGE-ASSETS.md`](docs/IMAGE-ASSETS.md) is generated by
`npm run images` and lists every slot, what fills it, what is still wanted, and
any file with a known problem — it is also the request for the next batch.

A slot with no file keeps a placeholder naming what it wants, which is the
honest state while artwork is still being made. QA check 18 fails on a
reference whose case does not match, because that works locally and 404s once
deployed.
